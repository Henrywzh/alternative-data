...
| Commercial aerospace | `build_hk_commercial_aerospace_artifact.py` | SSE/SZSE IPO status, exact-provider Launch Library 2 events and monthly cadence, CelesTrak constellation snapshots/history, FAA commercial-space KPIs, USAspending contracts, SEC company filings, global objects-launched benchmark, patents | Launch Library 2 persists normalized event history but may still use stale raw snapshots under its free-tier rate limit; CelesTrak counts tracked objects rather than confirmed operational satellites; SZSE's classification includes aviation/rail/other transport equipment; FAA KPIs are current snapshots rather than a historical event series; SEC data is filing-event discovery only and does not infer order/financing amounts; global benchmark counts objects/payloads, not rocket launches; Google Patents is currently unavailable. |
