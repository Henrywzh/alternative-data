...
| Hong Kong transport | `build_hk_transport_artifact.py` | MTR patronage, Cathay/HKIA traffic, China listed airlines, TD Table 2.1 public transport passenger journeys by operator/mode, TD Table 4.1(a) private-car fleet by fuel type, TD Table 4.1(c) private-car net first registration, TD Table 4.1(e) monthly private-car first registration by make/fuel, latest private-car make/model detail, real-time parking vacancy | MTR service-type breakdown uses the latest ten years by date; total MTR/Cathay/airline histories retain their longer available coverage. Monthly and weekly series must keep their source cadence and year visible. The TD make/fuel series is the primary EV-registration time series; the make/model feed is latest-month detail only. Parking vacancy is a five-minute current snapshot; the collector appends real snapshots and the history chart appears after repeated runs. Exact vacant-space totals exclude vacancy types B/C and negative/no-data values. The three TD Table 2.1/4.1(a)/4.1(c) scrapers recompute TD's own published subtotals from their parts and refuse to write output that doesn't reconcile. EV fleet share is of the registered (not licensed) fleet; EV first-registration share is a monthly flow share. |
