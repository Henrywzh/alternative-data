...
Last verified against the tracked-wrapper Eastmoney fallback from a GitHub
Actions runner on 2026-09-23. The read-only probe returned all 36 registered
wrappers with valid prices and premium fields. Eastmoney does not provide a
source observation timestamp in this response, so retrieval time must not be
described as a verified quote observation time.
