...
- FY26 RECONCILIATION complete (2026-08-09): EPS bridge now reports dual EPS - underlying (recurrent + property, no IP reval) base 1.96 and reported base 2.36 with IP reval +2.5bn scenario (bear +0.5 / base +2.5 / bull +4.5bn), vs consensus 2.52 (-6.5% base, range covers consensus). CONCLUSION: FY26 earnings broadly well-understood by consensus; the previous 20-43% gap was an overly conservative IP-revaluation assumption, not core operations. Research horizon moves to FY27.
...
- FY27 property pool is live: `scripts/mtr_property_expected_profit_fy27.py` -> mtr_property_expected_profit_fy27.csv. FY27 expected property profit 4.7/6.2/7.8bn (bear/base/bull) - similar to FY26, implying no FY27 collapse under our pool. NOTE: yfinance +1y consensus EPS 1.55 implies FY27 property+IP of only ~30-35bn, well below our 62bn base - a potential long variant, but +1y data quality is unverified (yearAgoEps already proven wrong). Priorities: verify LP13 identity/value, P6 scale, YOHO WEST PARKSIDE scale.
