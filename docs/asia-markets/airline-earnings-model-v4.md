...
Status: 2026-08-10.  v4 replaces the flat-yield family as the primary
...
| + residual_yield (bounded) | 7.77% | 10.02% | 3.28% | 0.798 |
| + recovery_overlay (Spring-only) | **7.55%** | **9.69%** | 3.28% | 0.798 |
...
2. **dynamic_shrinkage**: the big win - MAE -1.2pp, regime-year MAE
   -1.5pp.  lambda_min tuned by walk-forward sweep (0.05..1.0): 0.5 is
   optimal.  More aggressive shrinkage (0.2) hurts BOTH MAE and rank IC
...
4. **recovery_overlay**: Spring-only pre-declared +10% yield premium when
   ...
   (`recovery_overlay_active`); fixes Spring 2023 (-27% -> -9.3%) with no
   harm to other carriers.  The ablation table answers the design
   question: it is worth keeping as a core overlay, not just a thesis
   layer - it fixes the single worst error row without sacrificing
   stability elsewhere.
...
- 2023 big-three errors improve to ~3-6% but Juneyao 2021/2022 (COVID
  ...
  persist.
- Rank IC drops from base 0.863 to final 0.798: shrinkage trades a little
