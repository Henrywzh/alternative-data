...
| P2 | Wikipedia airline fleet tables | aircraft type, in-service/on-order counts, seats; revision ID/timestamp as PIT proxy | Implemented in `airline_fleet_wikipedia_snapshot.csv` (45 rows, six mainland carriers, 2026-08-10) and carried into v3 as fleet context | Community-maintained and can lag filings; Cathay page is an image; never overrides official fleet totals |
