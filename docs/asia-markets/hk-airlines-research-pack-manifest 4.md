...
| Provisional trade-thesis scenarios | `airline_pair_trade_thesis_scenarios.csv` | 15 bear/base/bull rows combining model revenue gaps with free 3Y median annual-P/S reversion diagnostics, model-only comparison returns, directional beta hedge, payoff/drawdown, catalysts and risk rules | Annual-P/S history is mostly period-end-only and current TTM versus annual basis is not fully aligned; valuation reversion is diagnostic rather than final fair value |
| Valuation / factor review | `airline_pair_valuation_factor_review.csv` | Five-pair stress test of historical-P/S reversion, long-leg multiple compression, factor gaps, HK/A consensus scope and valuation readiness; current snapshot keeps all five not trade-ready | Historical-P/S reversion cannot bypass the scope/PIT/1H2026 gates; this is an explicit rejection/validation gate, not a trade recommendation or proof that the pair has no future opportunity |
