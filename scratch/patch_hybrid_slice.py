...
start_marker = "# --- Revenue Estimator Logic ---"
end_marker = "def render_leaderboard("
...
# Grab the spacing before render_leaderboard so we keep it clean
end_spacing = content[end_idx-4:end_idx]
...
new_block = """# --- Revenue Estimator Logic ---
    ...
    if activity_res and not activity_res.frame.empty and pricing_res and not pricing_res.frame.empty:
        ...
        def normalize_slug(slug):
            slug_str = str(slug)
            if str(slug_str) == "nan" or str(slug_str).lower() == "none" or not slug_str:
                return slug_str
...
new_content = content[:start_idx] + new_block + end_spacing + content[end_idx:]

with open("dashboard/app.py", "w") as f:
    f.write(new_content)

print("Patch applied using substring slice method.")
