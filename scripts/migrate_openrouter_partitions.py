#!/usr/bin/env python3
"""Split the single-file openrouter datasets in PARTITION_COLUMNS into date partitions.
...
compared cell-for-cell against it, so a failed migration leaves the original
in place rather than a half-written directory.
"""
...
from openrouter_data.storage import PARTITION_COLUMNS, StorageManager  # noqa: E402
...
def _columns_match(left: pd.Series, right: pd.Series) -> bool:
    ...
    # Treat as numeric only when coercion did not invent nulls on either side,
    # so a genuinely textual column still falls through to a string compare.
...
def migrate(base_dir: Path, *, dry_run: bool = False) -> int:
    ...
    for dataset_id in sorted(PARTITION_COLUMNS):
        ...
        columns = storage._dataset_columns(dataset_id)
        ...
        # Compare values, not their representation. The partition schema is
        # pinned (that is what keeps an unchanged partition byte-identical),
        # so an integer column comes back as float64 -- 10 reads as 10.0.
        # Stringifying both sides would flag that as data loss when nothing
        # was lost, so numeric columns are compared numerically.
        differing = [
            column
            for column in columns
            if not _columns_match(left[column], right[column])
        ]
        if differing:
