...
class CmeVoiPipeline:
    def __init__(self, base_dir: Path, client: CmeBulletinClient | None = None) -> None:
        self.base_dir = base_dir
        self.storage = CmeVoiStorage(base_dir)
        self.client = client or CmeBulletinClient()

    def run(self) -> dict[str, object]:
        run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + "-" + uuid4().hex[:8]
        logger.info("Starting CME VOI Ingestion Run: %s", run_id)
        ...
        raise NotImplementedError(
            "CmeVoiPipeline has no in-package collector. Run:\n"
            "  python3 scripts/backfill_free_institutional_data.py --sources cme"
        )
