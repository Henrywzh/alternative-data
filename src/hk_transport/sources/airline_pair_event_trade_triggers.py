...
def build_airline_pair_event_trade_triggers(
    *,
    ...
    for _, pair in working.iterrows():
        ...
        half_profit_advantage = abs(profit_advantage) / 2.0 if profit_advantage is not None else None
        half_revenue_advantage = abs(revenue_advantage) / 2.0 if revenue_advantage is not None else None
        ...
        rows.append(
            {
                ...
                "source_paths": f"{WORKING_PATH};{TRADE_PATH};{DIRECTION_PATH};{REVISION_PATH};{TARGET_PATH};{RISK_PATH}",
