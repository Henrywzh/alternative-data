from __future__ import annotations

import argparse
from pathlib import Path

from openrouter_data.pipeline import RankingsPipeline


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="OpenRouter rankings alternative-data pipeline")
    parser.add_argument("--base-dir", default=".", help="Repository root for data writes")
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("initial-backfill", help="Fetch full available history and write normalized outputs")
    subparsers.add_parser("weekly-update", help="Fetch current snapshots and append unseen completed weeks")
    subparsers.add_parser("backfill-missing", help="Fill all unseen completed weeks from the live source")

    validate = subparsers.add_parser("validate", help="Validate live extraction or parse a fixture snapshot")
    validate.add_argument("--fixture", help="Optional rankings fixture HTML path")
    validate.add_argument(
        "--programming-fixture",
        help="Optional programming rankings fixture HTML path. Defaults to --fixture when omitted.",
    )
    return parser


def main() -> None:
    parser = build_parser()
    args = parser.parse_args()
    pipeline = RankingsPipeline(Path(args.base_dir).resolve())

    if args.command == "initial-backfill":
        result = pipeline.run_initial_backfill()
        print(f"run_id={result.run_id}")
        for dataset_id, total_rows in result.datasets_written.items():
            print(f"{dataset_id}: {total_rows} rows written")
        print(f"raw_run_dir={result.raw_run_dir}")
        return

    if args.command == "weekly-update":
        result = pipeline.run_weekly_update()
        print(f"run_id={result.run_id}")
        for dataset_id, total_rows in result.datasets_written.items():
            print(f"{dataset_id}: {total_rows} rows written")
        print(f"raw_run_dir={result.raw_run_dir}")
        return

    if args.command == "backfill-missing":
        result = pipeline.run_backfill_missing()
        print(f"run_id={result.run_id}")
        for dataset_id, total_rows in result.datasets_written.items():
            print(f"{dataset_id}: {total_rows} rows written")
        print(f"raw_run_dir={result.raw_run_dir}")
        return

    if args.command == "validate":
        fixture_html = None
        programming_fixture_html = None
        if args.fixture:
            fixture_html = Path(args.fixture).read_text(encoding="utf-8")
            if args.programming_fixture:
                programming_fixture_html = Path(args.programming_fixture).read_text(encoding="utf-8")
        counts = pipeline.validate(fixture_html=fixture_html, fixture_programming_html=programming_fixture_html)
        for dataset_id, count in counts.items():
            print(f"{dataset_id}: {count} records")
        return

    parser.error(f"Unknown command: {args.command}")


if __name__ == "__main__":
    main()
