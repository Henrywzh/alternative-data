...
_ENTRY_KEYS = {"aa_model_id", "family_id", "openrouter_model_ids"}
...
def load_capability_map(base_dir: Path) -> CapabilityMap:
    ...
    for index, row in enumerate(rows):
        ...
        activity_ids = row["openrouter_model_ids"]
        ...
        if (
            ...
            or any(not isinstance(activity_id, str) or not activity_id for activity_id in activity_ids)
        ...
        entries.append(
            CapabilityEntry(
                ...
                family_id=family_id,
                openrouter_model_ids=frozenset(activity_ids),
            )
        )
    return CapabilityMap(methodology_version=methodology_version, entries=tuple(entries))


def compatible_activity_ids(capability_map: CapabilityMap, aa_model_id: str) -> frozenset[str]:
    """Return only the exact curated OpenRouter routes for an AA configuration."""
    entry = capability_map.by_aa_model_id.get(aa_model_id)
    return entry.openrouter_model_ids if entry is not None else frozenset()
