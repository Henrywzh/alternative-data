...
def test_current_historical_earnings_bridge_aligns_six_companies_and_periods() -> None:
    ...
    assert frame.groupby("company").size().eq(41).all()
    ...
    assert frame.loc[frame["company"].ne("Hainan Airlines Holdings"), "operating_month_count"].ge(3).all()
    core = frame["company"].ne("Hainan Airlines Holdings") | frame["period_end"].ne("2016-03-31")
    assert frame.loc[core, ["revenue_usd_mn", "operating_cost_usd_mn", "ask_mn_seat_km", "rpk_mn_passenger_km", "jet_fuel_avg_usd_per_gallon", "usd_cny_avg"]].notna().all().all()
    assert frame["current_ashare_detailed_fy2026_net_profit_usd_mn"].notna().all()
    assert frame["current_ashare_detailed_snapshot_date"].eq("2026-08-07").all()
    assert frame["current_hk_broker_snapshot_date"].notna().sum() == 123
    ...
    assert frame["source_quality"].eq("derived_multi_source_bridge").all()
    assert frame["financial_point_in_time_status"].eq("period_end_only_no_announcement_date").all()
